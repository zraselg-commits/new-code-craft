      ...
      <div className="relative z-10 container mx-auto px-4 md:px-6 py-12 lg:py-20">
        <div className="grid lg:grid-cols-2 gap-10 lg:gap-16 items-center">
          ...
          <div className="flex flex-col order-1">
            ...
            {/* Stats — values come from admin settings when available */}
            <div
              className="flex items-center gap-8 mt-14 animate-fade-in"
              style={{ animationDelay: "500ms" }}
            >
              {(() => {
                const parseStatVal = (raw: string | undefined, fallback: number) => {
                  if (!raw) return { end: fallback, suffix: "+" };
                  const num = parseInt(raw.replace(/\D/g, ""), 10);
                  const suffix = raw.replace(/[0-9]/g, "").trim();
                  return { end: isNaN(num) ? fallback : num, suffix: suffix || "+" };
                };
                const proj = parseStatVal(settings?.statsProjects, 50);
                const cli  = parseStatVal(settings?.statsClients, 30);
                const yrs  = parseStatVal(settings?.statsYears, 3);
                return (
                  <>
                    <HeroCountUp end={proj.end} suffix={proj.suffix} label={t.heroProjects} />
                    <div className="w-px h-10 bg-gray-200 dark:bg-white/10" />
                    <HeroCountUp end={cli.end} suffix={cli.suffix} label={t.heroClients} />
                    <div className="w-px h-10 bg-gray-200 dark:bg-white/10" />
                    <HeroCountUp end={yrs.end} suffix={yrs.suffix} label={t.heroYears} />
                  </>
                );
              })()}
            </div>
